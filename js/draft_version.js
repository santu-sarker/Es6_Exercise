...
// **************  html elements and global variable declaration area *****************
...
const edit_submit = document.getElementById("edit_submit"); // save edited data button
...
// ******** paginate variables
const per_page = 5;
let total_page = Math.ceil(user.length / per_page);
let current_position = 0;
let current_page = 1;

// ********************* functions section **************
//  ******************* row creating main function ***************
...
const load_user = (page) => {
  ...
  total_page = Math.ceil(user.length / per_page);
  // console.log(total_page);
  create_pagination(total_page);
...
let paginate_buttons = document.querySelectorAll(".paginate_btn"); // selecting all paginate button except prev and next
paginate_buttons.forEach((btn) => {
  btn.addEventListener("click", (event) => {
    let btn_parent = btn.parentElement.parentElement.children;
    for (let i = 1; i <= btn_parent.length - 2; i++) {
...
const prev_btn = document.getElementById("prev_btn"); //pagination previous button
prev_btn.addEventListener("click", (event) => {
  let childens = document.querySelectorAll(".paginate_btn"); //pagination page number button
...
const next_btn = document.getElementById("next_btn"); // pagination next button
...
delete_confirm.addEventListener("click", (event) => {
  ...
  user.splice(id - 1, 1); // as index start form 0
  console.log(user);
  load_user(current_page);
  delete_modal.hide();
});
